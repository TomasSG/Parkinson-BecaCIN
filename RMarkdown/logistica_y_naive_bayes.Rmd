...
datos_bayes_con_na %>% 
  select(is.numeric) %>% 
  map(~ ggplot(datos_bayes_con_na, aes(.x)) +
        geom_density())
```

Ahora comparamos los mismos resultados aplicando una transformación logarítmica a los datos.

```{r}
lista_graficos <- datos_bayes_con_na %>% 
  select(is.numeric) %>% 
  map2(, ~ ggplot(datos_bayes_con_na, aes(log(.x))) +
        geom_density() + 
         xlab(.y))

for(i in seq_along(lista_graficos)) {
  grafico <- lista_graficos[i] + xlab(names(lista_graficos)[i])
  print(ggrafico)
}
```
...
```{r}
indices_train_bayes <- createDataPartition(datos_bayes_sin_na$diagnostico_pro,
                                          p = PORCENTAJE_TRAIN,
                                          list = FALSE) %>% 
  as.vector()
...
# TODO: Probar distribución normal y reorganiar factores
