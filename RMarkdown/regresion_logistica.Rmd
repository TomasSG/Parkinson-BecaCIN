...
No se observa ningun coeficiente de correlación mayor/menor a .5/-.5. Ahora, buscamos multicolinealidad usando el VIF. Para esto creamos una regresión lineal múltiple para verficiar si existe multicolinealidad entre todas las variables predictoras. **¿Esta bien?**
...
Como la observación tiene un hatvalue de 0.02414641, que es mayor a `r 2 * 3 / nrow(datos_it1)` lo consideramos atípico. **¿No habría que eliminarlo, no?**
...
Ahora que tenemos un modelo definido, graficamos los residuos 

```{r}
df_residuos <- data.frame(d_residuos = residuals(modelo_it3, type = "deviance"),
                          residuos = modelo_it3$residuals,
                          valores_ajustados = modelo_it3$fitted.values)

ggplot(df_residuos, aes(x = valores_ajustados, y = abs(d_residuos))) + geom_point() + geom_hline(yintercept = 2, color = "red")
