...
```{r}
df_resumen_resultados <- data.frame(NroPredictoras = c(), 
                                    k = c(),
                                    Predictoras = c(), 
                                    AUROC = c())
...
for(i in seq_along(var_predictoras)){
    ...
    indices_train <- createDataPartition(df[[NOMBRE_VAR_RESPUESTA]], p = PORCENTAJE_TRAIN)[[1]]
    ...
    knn <- train(x = df_train %>% select(-NOMBRE_VAR_RESPUESTA),
                 y = df_train %>% select(NOMBRE_VAR_RESPUESTA),
                 method = METODO_KNN,
                 trControl = train_control, 
                 tuneGrid = grid)
    ...
    df_resumen_resultados <- df_resumen_resultados %>% 
      add_row(NroPredictoras = c(), k = knn$bestTune[[1]], Predictoras = c(), AUROC = c())
}
```
