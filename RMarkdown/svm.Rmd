...
Creamso el grid y el train control a usar en los modelos

```{r}
# Creamos el grid y el trControl
tr_control <- trainControl(method = CONTROL_CV, number = K_FOLD)
tune_grid <- expand.grid("C" = seq(C_MIN, C_MAX, 0.1))
```
...
```{r}
# Kernel lineal
svm_lineal <- entrenar_modelo(y_train, x_train, METODO_SVM_LINEAL, tr_control, tune_grid)

# Kerner radial
svm_radial <- entrenar_modelo(y_train, x_train, METODO_SVM_RADIAL)

# Kernel polynomial
svm_poly <- entrenar_modelo(y_train, x_train, METODO_SVM_POLYNOMIAL)
```

Por últmo creamos una función para la evaluación del modelo con la curva ROC

```{r}
graficar_roc <- function(modelo, y_test, x_test, type_predict = "prob"){
  # Predecimos lso valores
  y_predict <- predict(svm_lineal, x_test, type = type_predict)[,1]
  
  # Generamos el objeto ROC
  objeto_roc <- roc(y_test, y_predict)
...
Evaluamos los modelos mediante esta función

```{r}
graficar_roc(svm_lineal, y_test, x_test)
```
