...
datos_izq %>% count(ROW_ID) %>% filter(n > 1) %>% nrow()
datos_der %>% count(ROW_ID) %>% filter(n > 1) %>% nrow()
...
# Observamos que los datos son en su mayoría dupicados, entonces lo qe haces es quedarnos es quedarnos con una observación de cada ROW_ID y desechar los otros.
datos_der <- datos_der %>% distinct(ROW_ID)
...
kbl(pmap_dfr(lista_aux, medidas_resumen, remover_na = TRUE), 
...
g1 <- g0 + geom_histogram(aes(d1), bins = calcular_cant_bins(sum(!is.na(datos_it1$d1))), alpha = 0.4, color = "firebrick", fill = "white")
g2 <- g0 + geom_histogram(aes(d2), bins = calcular_cant_bins(sum(!is.na(datos_it1$d2))), alpha = 0.4, color = "firebrick", fill = "white") + 
  scale_x_continuous(breaks = seq(-200, 0, 100))
g3 <- g0 + geom_histogram(aes(d3), bins = calcular_cant_bins(sum(!is.na(datos_it1$d3))), alpha = 0.4, color = "firebrick", fill = "white")
g4 <- g0 + geom_histogram(aes(d4), bins = calcular_cant_bins(sum(!is.na(datos_it1$d4))), alpha = 0.4, color = "firebrick", fill = "white")
g5 <- g0 + geom_histogram(aes(d5), bins = calcular_cant_bins(sum(!is.na(datos_it1$d5))), alpha = 0.4, color = "firebrick", fill = "white")

arrange <- ggarrange(g1, g2, g3, g4, g5, nrow = 2, ncol = 3)

annotate_figure(arrange,top = text_grob("Histograma variables di", family = familia_texto, face = face_titulo, size = tamanio_titulo))
...
g1 <- g0 + geom_boxplot(aes(d1), color = "darkblue", fill = "blue", alpha = .6)
g2 <- g0 + geom_boxplot(aes(d2), color = "darkblue", fill = "blue", alpha = .6)
g3 <- g0 + geom_boxplot(aes(d3), color = "darkblue", fill = "blue", alpha = .6)
g4 <- g0 + geom_boxplot(aes(d4), color = "darkblue", fill = "blue", alpha = .6)
g5 <- g0 + geom_boxplot(aes(d5), color = "darkblue", fill = "blue", alpha = .6)

arrange <- ggarrange(g1, g2, g3, g4, g5, nrow = 2, ncol = 3)

annotate_figure(arrange, top = text_grob("Boxplot variables di", family = familia_texto, face = face_titulo, size = tamanio_titulo))
...
annotate_figure(arrange,top = text_grob("Boxplot con zoom", family = familia_texto, face = face_titulo, size = tamanio_titulo))
...
g1 <- g0 + geom_histogram(aes(edad), bins = calcular_cant_bins(sum(!is.na(datos_it1$edad))), 
                          alpha = 0.4, color = "firebrick", fill = "white", na.rm = TRUE)
...
arrange <- ggarrange(g1, g2, g3, g4, g5,  nrow = 3, ncol = 2)

annotate_figure(arrange,top = text_grob("Histograma variables de anios", family = familia_texto, face = face_titulo, size = tamanio_titulo))
