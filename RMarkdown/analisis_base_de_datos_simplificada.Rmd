...
datos_it1 <- datos_it0 %>%
  mutate(diagnostico = as.factor(diagnostico),
         ...
         anio_diagnostico = as.Date(anio_diagnostico, format = "%Y"),
         educacion = as.factor(educacion),
         trabajo = as.factor(trabajo),
         genero = as.factor(genero),
         estado_civil = as.factor(estado_civil),
         anio_medicacion = as.Date(anio_medicacion, format = "%Y"),
         anio_enfermedad = as.Date(anio_enfermedad, format = "%Y"),
...
# Barplot variables con fecha
g1 <- g0 + geom_bar(aes(anio_diagnostico),na.rm = TRUE)
g2 <- g0 + geom_bar(aes(anio_medicacion), na.rm = TRUE)
g3 <- g0 + geom_bar(aes(anio_enfermedad), na.rm = TRUE)

arrange <- ggarrange(g1, g2, g3, nrow = 1, ncol = 3)

annotate_figure(arrange,top = text_grob("Barplot variables fecha"))
...
* **anio_medicacion** tiene problemas con los 0s.
* **anio_enfermedad** y **anio_diagnostico** tienen muchos outliers.
...
# Buscamos gente con diagnóstico positivo pero sin anio_diagnostico o anio_enfermedad
nrow(datos_it1 %>% filter(diagnostico == 1) %>% filter(is.na(anio_diagnostico)))
nrow(datos_it1 %>% filter(diagnostico == 1) %>% filter(is.na(anio_enfermedad)))

# Buscamos gente que empezo a tomer medicacion antes o despúes del diagnostico pero que figuren con diagnostico 0
nrow(datos_it1 %>% filter(punto_medicacion == levels(punto_medicacion)[4]) %>% filter(diagnostico != 1))
nrow(datos_it1 %>% filter(punto_medicacion == levels(punto_medicacion)[3]) %>% filter(diagnostico != 1))
...
# Buscamos gente que fumo pero con anios_fumo igual a 0 o NA
nrow(datos_it1 %>% filter(fumo == levels(fumo)[2]) %>% filter(anios_fumo == 0 | is.na(anios_fumo)))
