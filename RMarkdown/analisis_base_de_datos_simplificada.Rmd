...
```{r}
kbl(table(DBS = datos_it2$educacion)) %>% 
  kable_classic_2(full_width = FALSE, font_size = 20)
```
...
```{r}
kbl(table(DBS = datos_it2$trabajo)) %>% 
  kable_classic_2(full_width = FALSE, font_size = 20)
```
...
```{r}
kbl(table(DBS = datos_it2$genero)) %>% 
  kable_classic_2(full_width = FALSE, font_size = 20)
```
...
datos_it2 %>% 
  ...
  xlab("") +
  ggtitle("Diagnosticos postivios/negativos\nsegún nivel de educación") +
  scale_x_continuous(labels = label_percent(2))
```
...
```{r}
table(datos_it2$trabajo, datos_it2$fumo)
```
...
datos_it2 %>% 
  ...
  ggplot(aes(x = anio_diagnostico, y = anio_enfermedad)) +
  geom_point() +
  geom_point(data = )
