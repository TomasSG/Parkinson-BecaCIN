...
dibujar_histograma <- function(datos, variable){
  datos %>% 
    ggplot(aes_string(variable)) +
    geom_histogram(bins = calcular_cant_bins(sum(!is.na(datos_it1[[variable]]))), 
                   alpha = 0.4, 
                   color = "firebrick", 
                   fill = "white") +
    ylab("")
...
graficos <- map(nombres_variables, dibujar_histograma, datos = datos_it2)
...
rm(dibujar_histograma, graficos, arrange)
```
